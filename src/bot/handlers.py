    ...
    try:
        ...
        for event in events:
            ...
            if "date" in start:
                ...
                date_time = start.get("dateTime", "")[:16]  # Get YYYY-MM-DDTHH:MM
                if date_time:
        ...
        for entry in schedule_data:
            ...
            if shift_info:
                try:
                    ...
                    created_events.append(f"• {shift_date.strftime('%a %d %b')}: {shift_code}")
                except Exception as e:
                    logger.error(f"Failed to create calendar event: {e}")
                    created_events.append(f"• {shift_date.strftime('%a %d %b')}: {shift_code} (⚠️ failed)")
            else:
                created_events.append(f"• {shift_date.strftime('%a %d %b')}: {shift_code} (unknown)")
